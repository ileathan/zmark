...
{
    return QCoreApplication::translate("zmark-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. zmark_de.qm (shortcut "de" needs to be defined in zmark.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. zmark_de_DE.qm (shortcut "de_DE" needs to be defined in zmark.qrc)
...
void BitmarkApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // zmark: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitmarkGUI::tr("A fatal error occurred. Zmark can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse zmark.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        QMessageBox::critical(0, QObject::tr("Zmark"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Zmark"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // zmark: links repeatedly have their payment requests routed to this process:
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Zmark Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
