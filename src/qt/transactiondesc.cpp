...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, int vout, int unit)
{
    ...
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitmarkUnits::formatWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitmarkUnits::formatWithUnit(unit, nValue) + "<br>";
            }
        ...
        else
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitmarkUnits::formatWithUnit(unit, -wallet->GetDebit(txin)) + "<br>";
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitmarkUnits::formatWithUnit(unit, wallet->GetCredit(txout)) + "<br>";
        }
    }
    ...
    // Message from normal zmark:URI (zmark:123...?message=example)
